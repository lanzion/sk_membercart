<template>
  <div>
    ...
    <div class="input">
      <input type="text" name="menName" placeholder="请输入姓名" v-model="isName">
    </div>
    <div class="btn">
  ...
  export default {
    ...
    methods: {
      ...
      nextStep() {
        ...
        this.emitName();
        this.$router.push({ path: '/editInfo/birthday' })
      },
      getLaodMemberInfo(){
        new MbrService().LaodMemberInfo({}).then(res=>{
          if (res.ErrorCode == 0){
            this.isName = res.Data.Name;
            this.isPortrait = res.Data.Sex
          }
        })
      }
